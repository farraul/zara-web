...
const ContainerCart = () => {
    ...
    const total = storage.reduce((acc, item) => acc + item.storageOptions.price, 0);
    const handleDeleted = (data: ProductSelected) => {
        const storageNew = storage.filter(
            (product) =>
                product.id !== data.id || product.storageOptions.price !== data.storageOptions.price
        );
        toast.success(`Se ha eliminado con éxito el teléfono: ${data.name} del carrito`)
    ...
    return (
        <section className="xl:min-h-[100vh-24rem] w-full flex-1 flex flex-col p-4">
            ...
            <div className={`space-y-6 w-full ${storage.length === 0 ? 'h-[50vh]' : ''}`}>
                {storage.map((item, index) => (
                    <div key={index} className="flex flex-row h-full items-start md:items-center gap-6 pb-6">
                        ...
                        <div className="flex-1 flex flex-col items-start w-fit font-light text-gray-700 h-full">
                            ...
                            <p className="mt-5">
                                {item.storageOptions.price.toFixed(0)} EUR
                            </p>
                            <button
